...
Codex Reset Credit Manager는 OpenAI Codex 초기화권(reset credits)을 위한 작고 단순한 커뮤니티 제작 Windows 도구입니다. 선택한 초기화권 한 장을 만료 약 5분 전에 자동으로 사용(redeem/consume)합니다. 몇 개의 Python 및 PowerShell 스크립트와 작은 Tkinter UI, Windows 작업 스케줄러, 로컬 Codex CLI app-server를 결합한 토이 프로젝트입니다.

Codex quota를 재설정하거나 늘리지 않고, quota-reset token을 만들지 않으며, `auth.json`을 읽거나 backend API를 직접 호출하지 않습니다.

영문 문서는 [README.md](README.md)에 있습니다.

![자동 사용이 켜져 있고 초기화권 사용 시각이 예약된 Codex Reset Credit Manager 창](docs/images/manager-ui-example-01.png)

*실제 Codex Reset Credit Manager 화면입니다.*
...
`ManagerSync`는 로그인할 때와 30분마다 실행됩니다. 계정, 시계, Codex CLI, 전체 초기화권 목록을 확인하고 예약을 최대 하나로 유지하며 초기화권을 직접 사용하지는 않습니다.

exact-ID one-shot 작업 하나가 조금 일찍 시작해 만료 약 T−5분에 사용을 처리합니다. 선택한 초기화권만 사용하며 다른 권한으로 대체하지 않습니다. 동기화가 30분마다 실행되므로 새 초기화권은 만료까지 약 **46분** 이상 남아 있을 때만 자동 발견을 보장합니다. Controller가 만료에 너무 가까운 시점에 발견하면 서두른 live 작업을 만들지 않습니다.

전역 Codex CLI가 변경되면 `ManagerSync`가 버전, 서명, 필요한 app-server 계약, 계정, 전체 초기화권 목록을 읽기 전용으로 자동 재검증합니다. 검증된 CLI 업데이트만 후속 예약에 승인합니다.
...
- 같은 Codex 계정의 자동 사용을 여러 PC에서 동시에 켜지 마십시오. 각 설치가 같은 초기화권을 두고 경쟁할 수 있습니다.

## 안전 원칙

- 계정 조회와 초기화권 사용은 Codex CLI가 제공하는 로컬 app-server만 이용합니다.
- Policy, 로그, UI, 알림은 원본 credit ID, 이메일 주소, token, 멱등 키를 저장하거나 표시하지 않습니다.
- 불완전하거나 모호한 정보, 계정 변경, 시계 문제, 비호환 CLI, 변경된 계약은 fail-closed 처리합니다.
- 초기화권 사용에는 미리 선택한 exact credit이 필요합니다. 불명 결과는 해당 초기화권이 만료되어 목록에서 사라질 때까지 장벽으로 남으며, controller가 추측하거나 더 늦은 초기화권으로 넘어가지 않습니다.
