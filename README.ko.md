...
Codex 초기화권 자동 사용 관리자는 커뮤니티가 만든 Windows 자동화 도구로, 선택된 OpenAI Codex 초기화권 한 장을 만료 약 5분 전에 자동으로 사용(redeem/consume)합니다. Python 및 PowerShell 스크립트, Windows 작업 스케줄러, 로컬 Codex CLI app-server를 fail-closed exact-credit 검증과 함께 사용합니다. 설치 후 시작 메뉴의 작은 관리창에서 **Start Automatic Use**를 한 번 선택하면 됩니다.
...
이 프로젝트에서 **redeem**은 지원되는 로컬 Codex app-server 계약을 통해 기존 초기화권을 consume한다는 사용자용 표현입니다. 이 도구는 Codex quota를 재설정하거나 늘리지 않고, quota-reset token을 생성하지 않으며, `auth.json`을 읽거나 backend API를 직접 호출하지 않습니다.
